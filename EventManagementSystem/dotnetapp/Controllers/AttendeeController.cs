...
namespace dotnetapp.Controllers
{
    ...
    public class AttendeeController : ControllerBase
    {
        ...
        // GET: api/Attendee/SortedByName
        [HttpGet("SortedByName")]
        public async Task<ActionResult<IEnumerable<Attendee>>> GetAttendeesSortedByName()
        {
            var sortedAttendees = await _context.Attendees
                .OrderByDescending(a => a.Name) // Sort attendees by Name in descending order
                .Include(a => a.Event) // Eager load Event
                .ToListAsync();

            return Ok(sortedAttendees);
        }
    }
}
